...
- `Request Inbox`: app-managed Vercel project/token deployment, health checks, allowed-origin evidence, and test requests.
...
When iCloud shared settings are enabled, native Booking setup roams across the user's installs through the same non-secret shared configuration path as calendar selections. That includes appointment type definitions, page and inbox URLs, GitHub/Vercel setup metadata, native profile/theme fields, selected appointment type, and automatic-approval preferences. It does not include Vercel account tokens, inbox admin tokens, booking private keys, slot-signing secrets, deploy-key private keys, generated page-file paths, or the editable HTML/CSS/template folder; those stay local to each device in the app credential vault.

GitHub Pages publishing is root-only once repository settings and a repository deploy key are configured. The app expects a dedicated empty repository, publishes generated booking artifacts at repository root over Git SSH, and blocks upload when the root already contains files outside the current generated artifact set. App Store macOS archives must not ship copied Apple Git/SSH command-line tools; app-managed GitHub publishing should either use a review-safe in-process implementation or fail clearly when no approved publishing engine is available. Manual publish and the macOS background polling loop both regenerate local booking artifacts for every active appointment type across each type's configured availability horizon, clone the repository with the stored deploy-key private key, skip byte-identical generated files, and commit/push only new or changed generated files when the publishing engine is present. If another app instance or agent changed generated booking files in the repository, the native app overwrites them with the current local source of truth and records a warning in the Booking evidence/audit trail instead of failing the publish as a conflict. iOS can generate and verify page files but does not perform app-managed GitHub publishing in this milestone.
...
- `allowedOrigin`
- `storage`
- `storageReady`, when a provider can report whether durable storage is attached
...
Guided Vercel setup is app-managed. The user enters a Vercel account token, a Vercel project ID or name, and an optional team ID or slug. The app stores the token locally, generates the inbox admin token locally, upserts the relay environment variables, deploys the bundled Vercel relay template through the Vercel REST API, saves the returned deployment URL as the inbox URL, checks `/healthz`, and sends/imports encrypted test requests. Vercel Blob storage must already be attached to the target project; the Vercel template reports a non-2xx health response until `BLOB_READ_WRITE_TOKEN` exists.
