...
- macOS app builds now package `booking-git`, `booking-ssh`, `booking-ssh-keygen`, and a minimal `booking-git-core` exec-path directory beside the app executable. The GitHub publisher sets `GIT_EXEC_PATH` to `booking-git-core` so the bundled Git does not fall back to Xcode or Command Line Tools on the user's Mac.
