...
- 2026-06-18: `/usr/bin/git` is only an `xcrun` shim on the current macOS environment, so copying it into the app would still depend on Command Line Tools. Xcode's real Git binary can be relocated only when the bundled `git-core` directory is also present and the app sets `GIT_EXEC_PATH` to that bundled directory.
...
- macOS app builds now package `booking-git`, `booking-ssh`, `booking-ssh-keygen`, and `booking-git-core` beside the app executable. The GitHub publisher sets `GIT_EXEC_PATH` to the bundled `booking-git-core` path so the relocated Git does not fall back to Xcode or Command Line Tools on the user's Mac.
