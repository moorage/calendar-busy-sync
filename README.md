...
- full-mesh mirroring is live for the selected calendars: any event that is not `free` / `available` becomes an opaque `Busy` hold on every other selected calendar, and moved or deleted source events reconcile on the next sync pass
- the reconciliation window is bounded to the past 24 hours plus the next 60 days, which keeps polling predictable while still catching recent edits and near-term scheduling changes
